...
#############################################################################
#################################config######################################
appName = "UART_AUTO_DETECT"
strUninstallApp = "uninstall_UART_AUTO_DETECT"
versionMajor = 1
versionMinor = 0
versionDev   = 0
author       = "Neucrack"
...
#中文需要显式用gbk方式编码
product_name = appName.encode('gbk')
unproduct_name = (strUninstallApp).encode('gbk')
product_desc = (appName+" V"+str(versionMajor)+"."+str(versionMinor)+"."+str(versionDev)).encode("gbk")

#uuid叫通用唯一识别码，后面再卸载快捷方式中要用到
product_code = msilib.gen_uuid()
#主程序手动命名
target_name= 'UART_AUTO_DETECT.exe'
...
setup(  name   = appName,
        author = author,
        version = str(versionMajor)+"."+str(versionMinor)+"."+str(versionDev),
